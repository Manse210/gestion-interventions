...
<template>
    ...
    <AppLayout>
        <div class="space-y-7">
            <h2 class="section-title text-[26px] text-main">Avis clients</h2>
            ...
            <div class="flex flex-wrap items-center gap-4">
                <div class="cyber-card rounded-[20px] p-5 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-[11px] text-muted uppercase tracking-wider font-semibold">Note moyenne</p>
                        <p class="text-[22px] font-['Space_Grotesk'] font-bold text-main">{{ moyenneGlobale.toFixed(1) }} <span class="text-[14px] text-muted">/5</span></p>
                    </div>
                </div>
            ...
            <p v-if="technicienFilter && filteredEvaluations.length" class="text-[12px] text-muted">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                >
                    <div class="flex items-center justify-between mb-3">
                        <span class="text-[14px] font-semibold text-main">{{ evalItem.client?.name || 'Anonyme' }}</span>
                    ...
                    <p v-if="evalItem.commentaire" class="text-[13px] text-sub leading-relaxed flex-1">
                        &#8220;{{ evalItem.commentaire }}&#8221;
                    </p>
                    <p v-else class="text-[13px] text-muted italic flex-1">
                        Aucun commentaire.
                    </p>

                    <div class="text-[11px] text-muted mt-4 pt-3 border-t border-subtle flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            >
                <p class="text-[14px] text-muted">Aucun avis trouvé.</p>
            </div>
        </div>
    </AppLayout>
</template>
